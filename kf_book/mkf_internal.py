...
def _multivariate_gaussian(x, mu, cov):
    """NumPy 2.x 兼容版 multivariate_gaussian。

    filterpy 1.4.5 的 stats.multivariate_gaussian 使用了
    ``np.array(..., copy=False)``，该写法在 NumPy 2.0 起会因"禁止复制"
    而抛出 ValueError（把 list 转成 ndarray 必然需要复制）。这里改用
    np.asarray 复刻同样的数学逻辑，兼容 NumPy 1.x / 2.x。
    """
...
def plot_3d_covariance(mean, cov):
    ...
    zs = np.array([100.* _multivariate_gaussian(np.array([x,y]),mean,cov) \
...
def plot_3d_sampled_covariance(mean, cov):
    ...
    zs = np.array([100.* _multivariate_gaussian(np.array([xx,yy]),mean,cov) \
